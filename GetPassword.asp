...
<table align='center' width='300' border='0' cellpadding='4' cellspacing='0' class='border'>
  ...
  <tr> 
    <td height='100' colspan='2' align="center" class='tdbg'><form name="form1" method="post" action="">
        ...
        <input name="Cancel" type="button" id="Cancel" style="cursor:hand;" onclick="javascript:window.close();" value=" 取消 ">
      </form></td>
  </tr>
</table>
<%
elseif Action="step2" then
	...
	if FoundErr=true then
		...
		rsGetPassword.open "select UserName,Question,Answer,Password from Userinfo where UserName='" & UserName & "'",conn,1,1
...
elseif Action="step4" then
	...
	if FoundErr=True then
		...
		set rsGetPassword=server.createobject("adodb.recordset")
		rsGetPassword.open "select UserName,Question,Answer,Password from Userinfo where UserName='" & UserName & "'",conn,1,3
		if rsGetPassword.bof and rsGetPassword.eof then
			FoundErr=True
			ErrMsg=ErrMsg & "<br><li>对不起，用户名不存在！可能已经被管理员删除了。</li>"
			call WriteErrMsg()
...
<table align='center' width='300' border='0' cellpadding='4' cellspacing='0' class='border'>
  ...
  <tr> 
    <td height='100' colspan='2' align="center" class='tdbg'> <table width="90%" border="0" cellspacing="5" cellpadding="0">
      ...
      <br>
      <font color="#FF0000">请记住您的新密码并使用新密码<a href="index.asp" target="_blank">登录</a>！</font><br> 
      <br>【<a href="javascript:window.close();">关闭窗口</a>】</td>
  </tr>
</table>
<%
			end if
