        ...
        <tr>
          <td><table width="776"  border="0" cellspacing="0" cellpadding="0">
		   ...
		   %>
			...
			%>
 <%
					dim strFileName,splb,spgqlb,SmallClassName,spname
					splb=ReplaceBadChar(request("splb"))
					spgqlb=ReplaceBadChar(request("spgqlb"))
					SmallClassName=ReplaceBadChar(request("SmallClassName"))
					spname=ReplaceBadChar(request("spname"))
					const MaxPerPage=18
					dim totalPut,CurrentPage,TotalPages,UserName
					dim i,j
					strFileName="spsearch.asp?splb="&splb&"&spgqlb="&spgqlb&"&SmallClassName="&SmallClassName&"&spname="&spname
					dim strPurview
					...
					sql="select * from spw where 1=1"
					if splb="" and spgqlb="" and SmallClassName="" and spname="" then
					 sql=sql
					else
					  if splb<>"" then
					  ...
					  if spname<>"" then
					   sql=sql&" and spname like '%"&spname&"%'"
					  end if
					end if
              ...
              <td width="596" valign="top"><table width="100%"  border="0" cellspacing="0" cellpadding="5">
                <tr>
                  <td>&nbsp;<a href="index.asp">返回首页</a>&nbsp;&gt;&gt;&gt;&nbsp;商铺搜索结果
                    <%if splb<>"" then response.write "&nbsp;&gt;&gt;&gt;&nbsp;"&splb%>                    
                    <%if spgqlb<>"" then response.write ","&spgqlb%>                    
                    <%if SmallClassName<>"" then response.write ","&SmallClassName%>                    
                    <%if spname<>"" then response.write "&nbsp;<font color=red>关键字:</font>"&spname%></td>
                </tr>
              </table>
...
%>
                      <TABLE width=100% border=0 align="center" cellPadding=0 cellSpacing=0 style="FONT-SIZE: 12px">
                        <TBODY>
                          ...
                          <tr>
                            <td width="20%"><a href="sqdetails.asp?id=<%=rs("id")%>" target="_blank"><img src="<%=rs("spphoto")%>" border="1" class="img-75_85"></a></td>
                            <td width="80%" height="80" valign="top" bgcolor="#F5F5F5" class="td-tianchong-4px"><b><%=rs("splb")%></b><br>
