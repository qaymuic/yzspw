...
Class Dv_Forum_Admin
	...
	Sub GetUserID()
		Dim Rs
		Set Rs=Dvbbs.Execute("Select PostUserid,UserName From "&TotalUseTable&" Where announceID="&replyID&"")
		If Not Rs.EOF Then
			TopicUserID=Rs(0)
			TopicUsername=Rs(1)
		End If 
	...
	Public Sub Tmove()
		...
		If Request("checked")="yes" Then
			...
			If Request("leavemessage")="yes" Then
				...
				newparentID=rs(0)
				sql="select * from "&TotalUseTable&" where rootID="&ID&" order by AnnounceID"
				set rs=Dvbbs.Execute(sql)
				do while not rs.eof
			...
			ElseIf Request("leavemessage")="no" Then
				...
				if rs("isbest")=1 then
					...
					Dvbbs.Execute("update "&TotalUseTable&" set boardid="&newboardid&",isbest=0 where rootid="&id)
					Dvbbs.Execute("delete from dv_besttopic where rootid="&id)
				else
					...
					Dvbbs.Execute("update "&TotalUseTable&" set boardid="&newboardid&" where rootid="&id)	
				end if
				...
				If Rs("istop") > 0 Then
					...
					Dvbbs.Execute(Sql)
					Dvbbs.ReloadBoardInfo(Newboardid)
				End If
				'批量移动上传文件数据
				dim F_announceID
				F_announceID=id & "|"
		...
		Else
			...
			Set Rs=Dvbbs.Execute("select boardID,boardtype,depth,Board_Setting from dv_board Where Not BoardID in (444,777) order by rootID,orders")
			Do While Not Rs.Eof
				reBoard_Setting=split(rs(3),",")
				BoardJumpList = BoardJumpList & "<option value="""&rs(0)&""" "
				...
				If rs(2)>1 Then
					...
					BoardJumpList = BoardJumpList & "&nbsp;&nbsp;├"
				End If
				BoardJumpList = BoardJumpList & rs(1)
				If Cint(reBoard_Setting(43))=1 Then
					BoardJumpList = BoardJumpList & "(不许转移)"
				End If
				BoardJumpList = BoardJumpList & "</option>"
			Rs.MoveNext
			Loop
			Set Rs=Nothing
	...
	Public Sub copy()
		...
		If Request("checked")="yes" Then
			...
			Else
				newtopic=Dvbbs.CheckStr(Request.form("topic"))
			End If
			sql="insert into dv_topic (Title,BoardID,PostUsername,PostUserID,DateAndTime,Expression,LastPost,LastPostTime,child,hits,isvote,isbest,votetotal,PostTable) values ('"&newtopic&"',"&newboardID&",'"&rs("username")&"',"&rs("postuserID")&","&SqlNowString&",'"&rs("Expression")&"','"&rs("username")&"$#$"&Now()&"$$$$',"&SqlNowString&",0,0,0,0,0,'"&Dvbbs.NowUseBBS&"')"
			Dvbbs.Execute(sql)
