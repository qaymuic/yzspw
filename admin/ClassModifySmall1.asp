...
rs.Open "Select * from a2 where SmallClassID="&SmallClassID&"",conn,1,3
	...
	if Action="Modify" then
		...
		if FoundErr<>True then
			...
			if SmallClassName<>OldSmallClassName then
				conn.execute "Update spw set SmallClassName='" & SmallClassName & "' where BigClassName='" & BigClassName & "' and SmallClassName='" & OldSmallClassName & "'"
	     	end if	
			call CloseConn()
    	 	Response.Redirect "ClassManage1.asp"
		end if
...
<body>
<form name="form1" method="post" action="ClassModifySmall1.asp">
  <p>&nbsp;</p>
